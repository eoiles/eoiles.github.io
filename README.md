...
- 「编码格式」默认收起。Unicode 原生映射是历史兼容模式，需要用户手动选择；选择下拉框不会改动内容。两个明确动作分别保留原文重新编码、保留编码重新解释。
- 「怎么读」默认收起，选择当前内容中的码元，以高、低字节和实际点位解释；播放由用户主动触发。emoji 可能占两个或更多码元。
...
统计中的「可见字符」指用户感知的字素簇，包括空白字符；「码元」指 UTF-16 code unit；一个码元固定对应两个盲文字符。原文超过 12,000 码元时省略字素统计，明确显示「未计数」，转换不受影响。
